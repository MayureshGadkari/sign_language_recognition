...
while running:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            if count_int>9: #10 elements
                    print("\r\nChar Array:")
                    print(letter_array)
                    d = Counter(letter_array)
                    print(d)
                    new_list = ([item for item in d if d[item]>5]) # majority print[a=6,b=3,c=2]
                    mychar=(str(new_list)[1:-1]) #"a"
                    mychar=mychar.strip("")
                    if 'Back' not in letter_array:
                        #a
                        print(mychar)#<---changed
                    count_int=0
                    
                    #print(char_val)
                    if "clr" in mychar:#[a,a,a,clr]
                        string_array=""
                    elif "space" in mychar:
                        string_array+=" "
                    elif "Back" in mychar: #rahu
    ...
    cv2.putText(
        image,
        "String:" + str(string_array.strip(None)),#<---changed
        (10, 60),
        cv2.FONT_HERSHEY_SIMPLEX,
        1.0,
        (0, 0, 0),
    ...
    cv2.imshow("Smart Hands ProtoElectronics", image)
    time.sleep(0.5)
